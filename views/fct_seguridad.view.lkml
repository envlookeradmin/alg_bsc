  ...
  derived_table: {
    ...
    sql:
     ...
     SELECT * FROM (SELECT  ID_EVENTO,
      ...
      CENTRO,
      TIPO_EVENTO ,
      1 valor FROM `envases-analytics-qa.RPT_EHS_MX_UPG.vw_bsc_reporte_seguridad` where  TITULO like '%ADP%'
      ...
      '' TIPO_EVENTO,
       0 valor
      FROM `envases-analytics-qa.RPT_EHS_MX_UPG.vw_bsc_reporte_seguridad` s
      CROSS JOIN  (select DATE from `envases-analytics-qa.ENVASES_REPORTING_UPG.CALENDAR`) c
