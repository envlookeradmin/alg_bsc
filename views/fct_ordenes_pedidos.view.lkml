view: fct_ordenes_pedidos {
  ...
  sql_table_name: `envases-analytics-qa.RPT_S4H_MX.tbl_fact_ordenes_pedidos`;;


  filter: date_filter {
    type: date
    sql: CAST(fecha_entrega_planeada AS date) >= (case when CAST({% date_start date_filter %} AS DATE)= DATE_ADD(CAST({% date_end date_filter %} AS DATE), INTERVAL -1 DAY) then DATE_TRUNC(CAST({% date_start date_filter %} AS DATE), YEAR) else CAST({% date_start date_filter %} AS DATE) end )
...
#
  ...
  # dimension_group: fecha_entrega_planeada {
  #   label: "fecha_entrega_planeada"
  #   type: time
  #   timeframes: [
  #     raw,
  #     time,
  ...
  #     year
  #   ]
  #   sql: CAST(${TABLE}.FECHA_ENTREGA_PLANEADA AS TIMESTAMP) ;;
  #
  #
  #   }


  #dimension: Periodo_filtro{
  #  hidden: yes
  #  type: yesno
  #  sql: CAST(${fecha_entrega_planeada_date} AS date) >= (case when CAST({% date_start date_filter %} AS DATE)= DATE_ADD(CAST({% date_end date_filter %} AS DATE), INTERVAL -1 DAY) then DATE_TRUNC(CAST({% date_start date_filter %} AS DATE), YEAR) else CAST({% date_start date_filter %} AS DATE) end )
  #        and CAST(${fecha_entrega_planeada_date} AS date) <= (case when CAST({% date_start date_filter %} AS DATE)= DATE_ADD(CAST({% date_end date_filter %} AS DATE), INTERVAL -1 DAY) then LAST_DAY(CAST({% date_start date_filter %} AS DATE)) else CAST({% date_end date_filter %} AS DATE) end ) ;;
#
  #}
