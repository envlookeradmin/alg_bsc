view: fct_ordenes_pedidos_anual {
 derived_table: {
  sql: SELECT * FROM `@{GCP_PROJECT}.@{REPORTING_DATASET}.vw_fact_ordenes_pedidos`
       WHERE DATE_TRUNC(CAST(fecha_entrega_planeada AS DATE),DAY) >=DATE_TRUNC(CAST({% date_start date_filter %} AS DATE), year) AND DATE_TRUNC(CAST(fecha_entrega_planeada AS DATE),DAY) <= DATE_ADD((CAST({% date_start date_filter %} AS DATE)),INTERVAL -0 day)
